...
namespace CryptoSymWPF
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private static void EncryptData(String inName, String outName, byte[] EnKey, byte[] EnIV, Alg SymAlg, CipherMode Cm, PaddingMode Pm, bool crypt)
        {
            ...
            try
            {
                SymmetricAlgorithm cypher = SymmetricAlgorithm.Create(SymAlg.ToString()); //Creates the default implementation, which is RijndaelManaged.
                cypher.Mode = Cm;
                cypher.Padding = Pm;
                ...
                if (crypt == true)
                {
                    cryptStream = new CryptoStream(fout, cypher.CreateEncryptor(EnKey, EnIV), CryptoStreamMode.Write);
                }
                else
                {
                    cryptStream = new CryptoStream(fout, cypher.CreateDecryptor(EnKey, EnIV), CryptoStreamMode.Write);
                }
